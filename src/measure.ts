...
/**
 * Тарифы: цена за месяц, сколько глав в квоте и во сколько глав квоты
 * обходится кнопка «перевести получше».
 *
 * Квоты урезаны вдвое, когда основой стал Sonnet. На дешёвой модели глава
 * обходилась в 2.87 ₽, на средней — в 5.75 ₽, и прежние сто глав за 490 ₽
 * ушли бы в минус. Цену не трогали: 490 и 1190 на витрине уже читаются как
 * пара, а квота — цифра, которую видно только внутри.
 *
 * Множитель тройной на обоих тарифах. С урезанной квотой глава квоты стоит
 * дороже, и на кнопку наконец встаёт Opus: 24.50 ₽ выручки против 14.58 ₽
 * себестоимости. Раньше он не окупался ни при каком множителе, который
 * читатель согласится нажать.
 */
const PLANS = [
  { name: "Читатель", rub: 490, chapters: 60, strongMultiplier: 3 },
  { name: "Запойный", rub: 1190, chapters: 150, strongMultiplier: 3 },
] as const;

/**
 * Окупается ли модель по кнопке при заявленном множителе.
 *
 * Считается отдельно, потому что убыток тут прячется: у «Запойного» глава
 * квоты дешевле, и один и тот же множитель приносит там заметно меньше.
 */
/**
